...
begin
  if pg_catalog.to_regclass('public.conversation_participants') is null
    ...
    raise exception 'Ember requires the premium entitlement and personal conversation theme migrations through 202608270003.';
  end if;
  ...
  if not exists (
    ...
    where billing_product.product_id = 'theme.sakura'
      and billing_product.premium_product_id = 'theme.sakura'
  ) then
    raise exception 'Ember requires the current Sakura theme and billing migration state.';
  end if;

  if exists (
    select 1
    from private.development_feature_flags as feature_flag
    where feature_flag.flag_key = 'ember_theme_preview'
  ) or exists (
    select 1
    from private.premium_products as product_row
    where product_row.product_id = 'theme.ember'
  ) or exists (
    select 1
    from private.billing_product_catalog as billing_product
    where billing_product.product_id = 'theme.ember'
      or billing_product.premium_product_id = 'theme.ember'
  ) then
    raise exception 'Ember is already registered; verify migration history before retrying.';
  end if;
end;
$preflight$;

-- Development preview is private, disabled by default, and remains separate
-- from permanent ownership, Gold account status, Elite, and billing state.
insert into private.development_feature_flags (flag_key, enabled)
values ('ember_theme_preview', false);

-- Register only the premium entitlement identity. Ember intentionally has no
-- billing_product_catalog row until its later Lemon Squeezy milestone.
insert into private.premium_products (product_id, development_preview_flag_key)
values ('theme.ember', 'ember_theme_preview');
...
do $verify$
begin
  if (select pg_catalog.count(*) from private.premium_products where product_id = 'theme.ember') <> 1
    or (select pg_catalog.count(*) from private.development_feature_flags where flag_key = 'ember_theme_preview' and not enabled) <> 1 then
    raise exception 'Ember entitlement registration verification failed.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Ember must not receive a billing catalog entry before provider setup.';
  end if;
end;
$verify$;
...
-- where product_id = 'theme.ember';
--
-- select flag_key, enabled
-- from private.development_feature_flags
-- where flag_key = 'ember_theme_preview';
--
-- select product_id, premium_product_id
-- from private.billing_product_catalog
-- where product_id = 'theme.ember' or premium_product_id = 'theme.ember';
-- Expected: no rows until the later Ember Lemon Squeezy integration.
--
