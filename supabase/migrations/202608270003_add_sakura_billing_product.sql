...
begin
  if pg_catalog.to_regclass('private.premium_products') is null
    ...
    raise exception 'Sakura billing requires the billing foundation and Sakura theme migrations through 202608270002.';
  end if;
  ...
  if not exists (
    ...
    where product_row.product_id = 'theme.sakura'
      and product_row.development_preview_flag_key = 'sakura_theme_preview'
  ) then
    raise exception 'The canonical Sakura premium product is not registered.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Sakura billing is already registered; verify migration history before retrying.';
  end if;
end;
$preflight$;
...
begin
  if (
    ...
    raise exception 'Sakura billing catalog verification failed.';
  end if;
end;
$verify$;
