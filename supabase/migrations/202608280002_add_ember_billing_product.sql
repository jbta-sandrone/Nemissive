...
begin
  if pg_catalog.to_regclass('private.premium_products') is null
    ...
    raise exception 'Ember billing requires the billing foundation and Ember theme migrations through 202608280001.';
  end if;
  ...
  if not exists (
    ...
    where product_row.product_id = 'theme.ember'
      and product_row.development_preview_flag_key = 'ember_theme_preview'
  ) then
    raise exception 'The canonical Ember premium product is not registered.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Ember billing is already registered; verify migration history before retrying.';
  end if;
end;
$preflight$;
...
begin
  if (
    ...
    raise exception 'Ember billing catalog verification failed.';
  end if;
end;
$verify$;
