...
begin
  if pg_catalog.to_regclass('private.premium_products') is null
    ...
    raise exception 'Dune billing requires the billing foundation and Dune theme migration through 202608300005.';
  end if;
  ...
  if not exists (
    ...
    raise exception 'The canonical Dune development-preview flag is not registered.';
  end if;
  ...
  if not exists (
    ...
    raise exception 'The canonical Dune premium product is not registered.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Dune billing is already registered; verify migration history before retrying.';
  end if;
end;
$preflight$;
...
begin
  if (
    ...
    raise exception 'Dune billing catalog verification failed.';
  end if;
end;
$verify$;
