...
begin
  if pg_catalog.to_regclass('private.premium_products') is null
    ...
    raise exception 'Verdant billing requires the billing foundation and Verdant theme migrations through 202608290001.';
  end if;
  ...
  if not exists (
    ...
    raise exception 'The canonical Verdant premium product is not registered.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Verdant billing is already registered; verify migration history before retrying.';
  end if;
end;
$preflight$;
...
begin
  if (
    ...
    raise exception 'Verdant billing catalog verification failed.';
  end if;
end;
$verify$;
