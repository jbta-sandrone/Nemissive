...
begin
  if pg_catalog.to_regclass('public.conversation_participants') is null
    ...
    raise exception 'Eclipse requires the premium entitlement and personal conversation theme migrations through 202608300002.';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Eclipse requires the current Abyss theme and billing migration state.';
  end if;

  if exists (
    select 1
    from private.development_feature_flags as feature_flag
    where feature_flag.flag_key = 'eclipse_theme_preview'
  ) or exists (
    select 1
    from private.premium_products as product_row
    ...
    where billing_product.product_id = 'theme.eclipse'
      or billing_product.premium_product_id = 'theme.eclipse'
  ) then
    raise exception 'Eclipse is already registered; verify migration history before retrying.';
  end if;
end;
$preflight$;
...
values ('eclipse_theme_preview', false);

-- Register Eclipse with the centralized premium resolver only. Permanent
...
values ('theme.eclipse', 'eclipse_theme_preview');
...
do $verify$
begin
  if (select pg_catalog.count(*) from private.premium_products where product_id = 'theme.eclipse' and development_preview_flag_key = 'eclipse_theme_preview') <> 1
    or (select pg_catalog.count(*) from private.development_feature_flags where flag_key = 'eclipse_theme_preview' and not enabled) <> 1 then
    raise exception 'Eclipse entitlement registration verification failed.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Eclipse must not receive a billing catalog entry before provider setup.';
  end if;
end;
$verify$;
