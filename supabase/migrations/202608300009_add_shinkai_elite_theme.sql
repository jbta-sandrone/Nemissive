...
begin
  if pg_catalog.to_regclass('public.conversation_participants') is null
    ...
    raise exception 'Shinkai requires the premium entitlement and personal conversation theme migrations through 202608300008.';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Shinkai requires the current Void theme and billing migration state.';
  end if;

  if exists (
    select 1
    from private.development_feature_flags as feature_flag
    where feature_flag.flag_key = 'shinkai_theme_preview'
  ) or exists (
    select 1
    from private.premium_products as product_row
    where product_row.product_id = 'theme.shinkai'
  ) or exists (
    select 1
    from private.billing_product_catalog as billing_product
    where billing_product.product_id = 'theme.shinkai'
      or billing_product.premium_product_id = 'theme.shinkai'
  ) then
    raise exception 'Shinkai is already registered; verify migration history before retrying.';
  end if;
end;
$preflight$;
...
values ('shinkai_theme_preview', false);

-- Register Shinkai with the centralized premium resolver only. Permanent
...
values ('theme.shinkai', 'shinkai_theme_preview');
...
do $verify$
begin
  if (select pg_catalog.count(*) from private.premium_products where product_id = 'theme.shinkai' and development_preview_flag_key = 'shinkai_theme_preview') <> 1
    or (select pg_catalog.count(*) from private.development_feature_flags where flag_key = 'shinkai_theme_preview' and not enabled) <> 1 then
    raise exception 'Shinkai entitlement registration verification failed.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Shinkai must not receive a billing catalog entry before provider setup.';
  end if;
end;
$verify$;
