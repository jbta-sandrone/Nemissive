...
begin
  if pg_catalog.to_regclass('private.premium_products') is null
    ...
    raise exception 'Eclipse billing requires the billing foundation and Eclipse theme migration through 202608300003.';
  end if;
  ...
  if not exists (
    ...
    raise exception 'The canonical Eclipse development-preview flag is not registered.';
  end if;
  ...
  if not exists (
    ...
    raise exception 'The canonical Eclipse premium product is not registered.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Eclipse billing is already registered; verify migration history before retrying.';
  end if;
end;
$preflight$;
...
begin
  if (
    ...
    raise exception 'Eclipse billing catalog verification failed.';
  end if;
end;
$verify$;
