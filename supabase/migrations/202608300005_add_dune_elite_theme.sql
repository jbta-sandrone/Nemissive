...
begin
  if pg_catalog.to_regclass('public.conversation_participants') is null
    ...
    raise exception 'Dune requires the premium entitlement and personal conversation theme migrations through 202608300004.';
  end if;

  if not exists (
    select 1
    from private.premium_products as product_row
    where product_row.product_id = 'theme.eclipse'
      and product_row.development_preview_flag_key = 'eclipse_theme_preview'
  ) or not exists (
    select 1
    from private.billing_product_catalog as billing_product
    where billing_product.product_id = 'theme.eclipse'
      and billing_product.premium_product_id = 'theme.eclipse'
  ) then
    raise exception 'Dune requires the current Eclipse theme and billing migration state.';
  end if;

  if exists (
    select 1
    from private.development_feature_flags as feature_flag
    where feature_flag.flag_key = 'dune_theme_preview'
  ) or exists (
    select 1
    from private.premium_products as product_row
    ...
    where billing_product.product_id = 'theme.dune'
      or billing_product.premium_product_id = 'theme.dune'
  ) then
    raise exception 'Dune is already registered; verify migration history before retrying.';
  end if;
end;
$preflight$;
...
values ('dune_theme_preview', false);

-- Register Dune with the centralized premium resolver only. Permanent
...
values ('theme.dune', 'dune_theme_preview');
...
do $verify$
begin
  if (select pg_catalog.count(*) from private.premium_products where product_id = 'theme.dune' and development_preview_flag_key = 'dune_theme_preview') <> 1
    or (select pg_catalog.count(*) from private.development_feature_flags where flag_key = 'dune_theme_preview' and not enabled) <> 1 then
    raise exception 'Dune entitlement registration verification failed.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Dune must not receive a billing catalog entry before provider setup.';
  end if;
end;
$verify$;
