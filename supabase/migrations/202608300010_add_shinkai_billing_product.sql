...
begin
  if pg_catalog.to_regclass('private.premium_products') is null
    ...
    raise exception 'Shinkai billing requires the billing foundation and Shinkai theme migration through 202608300009.';
  end if;
  ...
  if not exists (
    ...
    raise exception 'The canonical Shinkai development-preview flag is not registered.';
  end if;
  ...
  if not exists (
    ...
    where product_row.product_id = 'theme.shinkai'
      and product_row.development_preview_flag_key = 'shinkai_theme_preview'
  ) then
    raise exception 'The canonical Shinkai premium product is not registered.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Shinkai billing is already registered; verify migration history before retrying.';
  end if;
end;
$preflight$;
...
begin
  if (
    ...
    where billing_product.product_id = 'theme.shinkai'
      ...
      and billing_product.premium_product_id = 'theme.shinkai'
    ...
    raise exception 'Shinkai billing catalog verification failed.';
  end if;
end;
$verify$;
