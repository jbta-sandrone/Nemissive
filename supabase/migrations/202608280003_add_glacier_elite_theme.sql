...
begin
  if pg_catalog.to_regclass('public.conversation_participants') is null
    ...
    raise exception 'Glacier requires the premium entitlement and personal conversation theme migrations through 202608280002.';
  end if;
  ...
  if not exists (
    ...
    where billing_product.product_id = 'theme.ember'
      and billing_product.premium_product_id = 'theme.ember'
  ) then
    raise exception 'Glacier requires the current Ember theme and billing migration state.';
  end if;

  if exists (
    select 1
    from private.development_feature_flags as feature_flag
    where feature_flag.flag_key = 'glacier_theme_preview'
  ) or exists (
    select 1
    from private.premium_products as product_row
    ...
    raise exception 'Glacier is already registered; verify migration history before retrying.';
  end if;
end;
$preflight$;
...
values ('glacier_theme_preview', false);

-- Register Glacier with the centralized premium resolver only. Permanent
...
values ('theme.glacier', 'glacier_theme_preview');
...
do $verify$
begin
  if (select pg_catalog.count(*) from private.premium_products where product_id = 'theme.glacier') <> 1
    or (select pg_catalog.count(*) from private.development_feature_flags where flag_key = 'glacier_theme_preview' and not enabled) <> 1 then
    raise exception 'Glacier entitlement registration verification failed.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Glacier must not receive a billing catalog entry before provider setup.';
  end if;
end;
$verify$;
...
-- set enabled = true
-- where flag_key = 'glacier_theme_preview';
--
...
-- where product_id = 'theme.glacier' or premium_product_id = 'theme.glacier';
-- Expected: no rows until a future Glacier Lemon Squeezy integration.
--
