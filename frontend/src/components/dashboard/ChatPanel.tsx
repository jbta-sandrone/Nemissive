...
function AcceptedConversationPanel({ conversation, onMobileBack }: { conversation: SelectedConversation; onMobileBack: () => void }) {
  ...
  useEffect(() => {
    ...
    async function loadMessages() {
      const { data, error } = await supabase.from("messages").select("id, sender_id, body, created_at").eq("conversation_id", conversation.id).order("created_at", { ascending: true }).limit(100).abortSignal(abortController.signal);
      ...
      if (error) {
        setErrorMessage("We couldn’t load this conversation. Please try again.");
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-4 py-5 sm:px-6 lg:px-10">
        {isLoading ? (
          ...
          <div className="mx-auto max-w-2xl space-y-4">{messages.map((message) => { const isFromOther = message.sender_id === conversation.otherProfile.id; return <div key={message.id} className={`flex ${isFromOther ? "justify-start" : "justify-end"}`}><div className={`max-w-[85%] rounded-3xl px-4 py-3 shadow-soft sm:max-w-[75%] ${isFromOther ? "rounded-bl-md bg-surface text-body" : "rounded-br-md bg-primary text-white"}`}><p className="whitespace-pre-wrap break-words text-sm leading-6">{message.body}</p>{message.id === messages[0]?.id && <p className={`mt-2 text-xs ${isFromOther ? "text-muted" : "text-white/70"}`}>Introduction</p>}</div></div>; })}<p className="pt-4 text-center text-sm text-muted">No additional messages yet.</p></div>
...
function ChatPanel({ chatState, isMobileVisible, onStartConversation, onMobileBack }: ChatPanelProps) {
  ...
  if (chatState?.kind === "accepted") {
    return <main className={`${visibilityClasses} min-w-0 flex-1 flex-col overflow-hidden bg-background`}><AcceptedConversationPanel key={chatState.conversation.id} conversation={chatState.conversation} onMobileBack={onMobileBack} /></main>;
  }
