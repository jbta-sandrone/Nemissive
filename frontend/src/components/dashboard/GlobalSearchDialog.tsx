...
type LocalConversationResult =
  | { kind: "accepted"; id: string; profile: ProfileSearchResult; nickname: string | null; preview: string; timestamp: string; isArchived: boolean; conversation: SelectedConversation }
...
function getLocalResultName(result: LocalConversationResult) {
  return result.kind === "accepted" ? getConversationDisplayName(result.profile, result.nickname) : getProfileDisplayName(result.profile);
}
...
function GlobalSearchDialog({ currentProfile, conversations, outgoingRequests, incomingRequests, returnFocusRef, onClose, onConversationSelected, onOutgoingRequestSelected, onIncomingRequestSelected, onMessageSelected }: GlobalSearchDialogProps) {
  ...
  const localConversationResults = useMemo<LocalConversationResult[]>(() => {
    const accepted = conversations.map((conversation): LocalConversationResult => ({ kind: "accepted", id: conversation.conversationId, profile: conversation.otherProfile, nickname: conversation.otherNickname, preview: conversation.latestMessageIsDeleted ? "Message deleted" : conversation.latestMessage ?? "Conversation ready", timestamp: conversation.latestMessageAt ?? conversation.updatedAt, isArchived: Boolean(conversation.archivedAt), conversation: { id: conversation.conversationId, otherProfile: conversation.otherProfile, otherAccountStatus: conversation.otherAccountStatus, otherNickname: conversation.otherNickname, themeKey: conversation.themeKey, historyClearedAt: conversation.historyClearedAt, conversationDeletedAt: conversation.conversationDeletedAt } }));
    ...
    return allResults.filter((result) => getLocalResultName(result).toLocaleLowerCase().includes(loweredQuery) || result.profile.username?.toLocaleLowerCase().includes(loweredQuery) || result.preview.toLocaleLowerCase().includes(loweredQuery));
