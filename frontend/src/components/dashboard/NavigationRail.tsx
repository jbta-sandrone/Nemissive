...
const navigationItems: Array<{ section: DashboardSection; label: string }> = [
  ...
  { section: "archived", label: "Archived" },
...
function NavigationButton({ section, label, activeSection, pendingRequestCount, unreadMessageCount, archivedConversationCount, layoutId, showLabel, onSectionChange }: { section: DashboardSection; label: string; activeSection: DashboardSection; pendingRequestCount: number; unreadMessageCount: number; archivedConversationCount: number; layoutId: string; showLabel: boolean; onSectionChange: (section: DashboardSection) => void }) {
  ...
  const badgeCount = section === "requests" ? pendingRequestCount : section === "messages" ? unreadMessageCount : section === "archived" ? archivedConversationCount : 0;
  const badgeDescription = section === "requests" ? `${badgeCount} pending message ${badgeCount === 1 ? "request" : "requests"}` : section === "archived" ? `${badgeCount} archived ${badgeCount === 1 ? "conversation" : "conversations"}` : `${badgeCount} unread ${badgeCount === 1 ? "message" : "messages"}`;
  ...
  return (
    <button type="button" aria-label={accessibleLabel} aria-current={isActive ? "page" : undefined} title={label} onClick={() => onSectionChange(section)} className={`${showLabel ? "min-w-0 flex-1 flex-col gap-1 py-2" : "h-12 w-12"} relative flex items-center justify-center rounded-2xl text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-accent-hover ${isActive ? "text-white" : "text-muted hover:bg-accent hover:text-heading"}`}>
      ...
      {(section === "requests" || section === "messages" || section === "archived") && <CountBadge count={badgeCount} label={badgeDescription} compact={showLabel} />}
    </button>
  );
}

function NavigationRail({ activeSection, pendingRequestCount, unreadMessageCount, archivedConversationCount, isCompactChatVisible, onSectionChange, onSearch }: NavigationRailProps) {
  return (
    <>
      <aside className="hidden h-full w-16 shrink-0 flex-col items-center border-r border-border bg-background py-4 md:flex lg:w-[72px]" aria-label="Dashboard navigation">
        ...
        <nav className="mt-7 flex flex-col gap-2" aria-label="Dashboard sections">
          ...
          {navigationItems.map((item) => <NavigationButton key={item.section} {...item} activeSection={activeSection} pendingRequestCount={pendingRequestCount} unreadMessageCount={unreadMessageCount} archivedConversationCount={archivedConversationCount} layoutId="desktop-dashboard-section" showLabel={false} onSectionChange={onSectionChange} />)}
        </nav>
        <div className="mt-auto"><NavigationButton section="menu" label="Menu" activeSection={activeSection} pendingRequestCount={pendingRequestCount} unreadMessageCount={unreadMessageCount} archivedConversationCount={archivedConversationCount} layoutId="desktop-dashboard-section" showLabel={false} onSectionChange={onSectionChange} /></div>
      </aside>

      {!isCompactChatVisible && <nav className="order-last flex shrink-0 items-center gap-0.5 border-t border-border bg-surface px-1 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2 md:hidden" aria-label="Dashboard sections"><SearchNavigationButton showLabel onSearch={onSearch} />{[...navigationItems, { section: "menu" as const, label: "Menu" }].map((item) => <NavigationButton key={item.section} {...item} activeSection={activeSection} pendingRequestCount={pendingRequestCount} unreadMessageCount={unreadMessageCount} archivedConversationCount={archivedConversationCount} layoutId="mobile-dashboard-section" showLabel onSectionChange={onSectionChange} />)}</nav>}
    </>
  );
}
