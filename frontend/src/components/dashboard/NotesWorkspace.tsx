...
import { countWords, documentsEqual, emptyNoteDocument, noteDocumentMaxBytes, noteTextMaxLength, noteThemeIds, noteTitleMaxLength, type NoteThemeId } from "./noteDocuments";
...
type NotesWorkspaceProps = { currentUserId: string; returnFocusRef: RefObject<HTMLElement | null>; onClose: () => void };
...
function NotesWorkspace({ currentUserId, returnFocusRef, onClose }: NotesWorkspaceProps) {
  ...
  const update = useCallback((changes: Partial<EditorState>) => { const current = editorRef.current; if (!current) return; const next = { ...current, ...changes }; editorRef.current = next; setEditor(next); schedule(); }, [schedule]);
