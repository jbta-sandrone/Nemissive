...
function Sidebar({ activeSection, currentProfile, isAccountResolved, accountError, isCompactChatVisible, isDesktopCollapsed, requestsController, messagesController, chatState, onlineUserIds, quickReactions, onSaveQuickReactions, notificationPermission, isNotificationSupported, onEnableNotifications, onSaveNotificationPreferences, onProfileIdentityUpdated, onRequestSignOut, onAccountDeleted, onNewConversation, onPendingRequestSelected, onConversationReady, onPeopleConversationReady, onArchivedConversationReady, onConversationDeleted }: SidebarProps) {
  ...
  return (
    <aside className={`${visibilityClasses} ${desktopLayoutClasses} min-h-0 w-full min-w-0 flex-1 shrink-0 flex-col overflow-hidden border-r border-border bg-surface transition-[width,min-width,opacity] duration-200 motion-reduce:transition-none md:h-full md:w-auto md:flex-1`}>
