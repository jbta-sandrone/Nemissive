...
function Sidebar({ activeSection, currentProfile, isAccountResolved, accountError, isCompactChatVisible, isDesktopCollapsed, requestsController, messagesController, chatState, onlineUserIds, quickReactions, onSaveQuickReactions, notificationPermission, isNotificationSupported, onEnableNotifications, onSaveNotificationPreferences, onProfileIdentityUpdated, onRequestSignOut, onAccountDeleted, onNewConversation, onPendingRequestSelected, onConversationReady, onPeopleConversationReady, onArchivedConversationReady, onConversationDeleted, isUtilityShelfOpen, isUtilityLauncherHidden, onUtilityShelfToggle }: SidebarProps) {
  ...
  return (
    <aside className={`${visibilityClasses} ${desktopLayoutClasses} relative min-h-0 w-full min-w-0 flex-1 shrink-0 flex-col overflow-hidden border-r border-border bg-surface transition-[width,min-width,opacity] duration-200 motion-reduce:transition-none md:h-full md:w-auto md:flex-1`}>
      {showsWorkspaceUtility && <div className="pointer-events-none absolute bottom-[max(1rem,calc(env(safe-area-inset-bottom)+0.5rem))] right-4 z-20 sm:right-5"><UtilityShelfLauncher isOpen={isUtilityShelfOpen} onToggle={onUtilityShelfToggle} className="pointer-events-auto" /></div>}
