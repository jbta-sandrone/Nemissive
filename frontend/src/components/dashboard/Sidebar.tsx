...
function MessagesSidebarContent({ messagesController, chatState, onlineUserIds, onNewConversation, onPendingRequestSelected, onConversationReady }: { messagesController: MessagesDataController; chatState: DashboardChatState | null; onlineUserIds: ReadonlySet<string>; onNewConversation: () => void; onPendingRequestSelected: (request: PendingOutgoingRequest) => void; onConversationReady: (conversation: SelectedConversation) => void }) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <ConversationList pendingRequests={messagesController.pendingRequests} conversations={messagesController.conversations} searchQuery={searchQuery} selectedConversationId={chatState?.kind === "accepted" ? chatState.conversation.id : null} isLoading={messagesController.isLoading} loadError={messagesController.loadError} onlineUserIds={onlineUserIds} onRefresh={messagesController.refresh} onStartConversation={onNewConversation} onPendingRequestSelected={onPendingRequestSelected} onConversationSelected={(conversation) => onConversationReady({ id: conversation.conversationId, otherProfile: conversation.otherProfile })} onConversationPinned={messagesController.setConversationPinned} />
    </div>
  );
}

function Sidebar({ activeSection, currentProfile, isAccountResolved, accountError, isCompactChatVisible, requestsController, messagesController, chatState, onlineUserIds, quickReactions, onSaveQuickReactions, notificationPermission, isNotificationSupported, onEnableNotifications, onSaveNotificationPreferences, onBeforeSignOut, onNewConversation, onPendingRequestSelected, onConversationReady }: SidebarProps) {
