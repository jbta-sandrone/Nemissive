    ...
    <aside className={`${visibilityClasses} min-h-0 w-full min-w-0 flex-1 shrink-0 flex-col overflow-hidden border-r border-border bg-surface md:h-full md:w-auto md:flex-1 lg:w-80 lg:min-w-80 lg:flex-none xl:w-96 xl:min-w-96`}>
      <AnimatePresence mode="wait" initial={false}>
        <motion.div key={activeSection} initial={{ opacity: 0, x: shouldReduceMotion ? 0 : 8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: shouldReduceMotion ? 0 : -8 }} transition={transition} className="flex min-h-0 flex-1 flex-col overflow-hidden">
          ...
          {activeSection === "people" && <PeopleSidebarContent conversations={messagesController.relationshipConversations} isLoading={messagesController.isLoading} errorMessage={messagesController.loadError} selectedConversationId={chatState?.kind === "accepted" ? chatState.conversation.id : null} onlineUserIds={onlineUserIds} onRefresh={messagesController.refresh} onConversationReady={onPeopleConversationReady} onStartConversation={onNewConversation} />}
          ...
          {activeSection === "menu" && <MenuSidebarContent profile={currentProfile} isAccountLoading={!isAccountResolved} accountError={accountError} quickReactions={quickReactions} onSaveQuickReactions={onSaveQuickReactions} notificationPermission={notificationPermission} isNotificationSupported={isNotificationSupported} onEnableNotifications={onEnableNotifications} onSaveNotificationPreferences={onSaveNotificationPreferences} onBeforeSignOut={onBeforeSignOut} />}
        </motion.div>
      </AnimatePresence>
    </aside>
  );
