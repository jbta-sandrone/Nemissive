...
function VoiceRecordingComposer({ controller, isSending, shouldReduceMotion, onSend }: VoiceRecordingComposerProps) {
  ...
  if (controller.mode === "review" && controller.recording) {
    ...
    return (
      <div className="rounded-2xl bg-surface p-3 shadow-soft sm:p-4" aria-label="Review voice message">
        ...
        <div className="mt-3 flex flex-wrap items-center justify-end gap-2"><button type="button" onClick={() => void controller.rerecord()} disabled={isSending} className="inline-flex min-h-11 items-center justify-center rounded-xl border border-border bg-background px-3 py-2 text-sm font-semibold text-heading transition hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/20 disabled:cursor-wait disabled:opacity-60">Re-record</button><button type="button" onClick={controller.clearReview} disabled={isSending} className="inline-flex min-h-11 items-center justify-center rounded-xl border border-border bg-background px-3 py-2 text-sm font-semibold text-heading transition hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/20 disabled:cursor-wait disabled:opacity-60">Remove</button><button ref={primaryActionRef} type="button" onClick={onSend} disabled={isSending} className="inline-flex min-h-11 items-center justify-center rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-white shadow-soft transition hover:bg-primary-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/20 disabled:cursor-wait disabled:opacity-60">{isSending ? "Sending…" : "Send voice message"}</button></div>
  ...
  return (
    <div className="rounded-2xl bg-surface p-3 shadow-soft sm:p-4" aria-label="Voice recording controls">
