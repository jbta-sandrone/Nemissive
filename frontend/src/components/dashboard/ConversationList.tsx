...
function ConversationList({ pendingRequests, conversations, searchQuery, selectedConversationId, isLoading, loadError, onlineUserIds, onRefresh, onStartConversation, onPendingRequestSelected, onConversationSelected, onConversationPinned }: ConversationListProps) {
          ...
          <AnimatePresence initial>
            ...
            {filteredConversations.length > 0 && (
              <motion.section key="conversations" initial={{ opacity: 0, y: shouldReduceMotion ? 0 : 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: shouldReduceMotion ? 0 : -4 }} transition={transition} aria-labelledby="accepted-conversations-heading">
                ...
                <div className="space-y-1">{filteredConversations.map((conversation) => { const name = getProfileDisplayName(conversation.otherProfile); const isSelected = selectedConversationId === conversation.conversationId; const isOnline = onlineUserIds.has(conversation.otherProfile.id); const preview = conversation.latestMessageIsDeleted ? "Message deleted" : conversation.latestMessage ? `${conversation.latestMessageSentByCurrentUser ? "You: " : ""}${conversation.latestMessage}` : "Conversation ready"; const timestamp = conversation.latestMessageAt ?? conversation.updatedAt; const unreadDescription = conversation.unreadCount > 0 ? `, ${conversation.unreadCount} unread ${conversation.unreadCount === 1 ? "message" : "messages"}` : ""; const onlineDescription = isOnline ? ", online" : ""; const pinnedDescription = conversation.isPinned ? ", pinned" : ""; return <motion.div layout={!shouldReduceMotion} key={conversation.conversationId} className={`flex min-w-0 items-center rounded-2xl transition-colors ${isSelected ? "bg-accent shadow-soft" : "hover:bg-accent"}`}><button type="button" onClick={() => onConversationSelected(conversation)} aria-current={isSelected ? "true" : undefined} aria-label={`Open conversation with ${name}${pinnedDescription}${onlineDescription}${unreadDescription}`} className="flex min-w-0 flex-1 items-center gap-3 rounded-2xl p-3 text-left focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-accent-hover"><PresenceAvatar profile={conversation.otherProfile} isOnline={isOnline} /><div className="min-w-0 flex-1"><div className="flex min-w-0 items-center gap-1.5"><p className="truncate font-semibold text-heading">{name}</p>{conversation.isPinned && <PinIndicator />}</div><p className="mt-1 truncate text-sm text-body">{preview}</p></div><div className="flex max-w-16 shrink-0 flex-col items-end gap-2 self-start pt-1"><time dateTime={timestamp} className="truncate text-right text-xs leading-5 text-muted">{formatSidebarTime(timestamp)}</time>{conversation.unreadCount > 0 && <span className="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-primary px-1.5 text-[10px] font-bold text-white"><span aria-hidden="true">{conversation.unreadCount > 99 ? "99+" : conversation.unreadCount}</span><span className="sr-only">{conversation.unreadCount} unread {conversation.unreadCount === 1 ? "message" : "messages"}</span></span>}</div></button><div className="shrink-0 pr-2"><ConversationPinMenu conversationName={name} isPinned={conversation.isPinned} onChange={(pinned) => onConversationPinned(conversation.conversationId, pinned)} /></div></motion.div>; })}</div>
              </motion.section>
            )}
          </AnimatePresence>
        </div>
