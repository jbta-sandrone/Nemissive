...
function PeopleSidebarContent({ conversations, isLoading, errorMessage, selectedConversationId, onlineUserIds, onRefresh, onConversationReady, onStartConversation }: PeopleSidebarContentProps) {
  ...
  const people = useMemo(() => {
    ...
    return [...peopleByUserId.values()].sort((first, second) => getConversationDisplayName(first.otherProfile, first.otherNickname).localeCompare(getConversationDisplayName(second.otherProfile, second.otherNickname), undefined, { sensitivity: "base" }));
  }, [conversations]);
  const filteredPeople = useMemo(() => {
    const normalizedQuery = searchQuery.trim().toLocaleLowerCase();
    if (!normalizedQuery) return people;
    return people.filter(({ otherProfile, otherNickname }) => getConversationDisplayName(otherProfile, otherNickname).toLocaleLowerCase().includes(normalizedQuery) || otherProfile.username?.toLocaleLowerCase().includes(normalizedQuery));
  }, [people, searchQuery]);
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <div aria-busy={isLoading} className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-3 pb-20 sm:px-4">
        {isLoading ? (
          ...
          <div className="space-y-1">{filteredPeople.map((conversation) => {
            ...
            const isOnline = conversation.messagingAvailable && onlineUserIds.has(profile.id);
            return <button key={profile.id} type="button" onClick={() => onConversationReady({ id: conversation.conversationId, otherProfile: profile, otherAccountStatus: conversation.otherAccountStatus, historyClearedAt: conversation.historyClearedAt, conversationDeletedAt: conversation.conversationDeletedAt, otherNickname: conversation.otherNickname, themeKey: conversation.themeKey, connectionStatus: conversation.connectionStatus, iBlocked: conversation.iBlocked, interactionAllowed: conversation.interactionAllowed, messagingAvailable: conversation.messagingAvailable, requestAvailable: conversation.requestAvailable })} aria-current={isSelected ? "true" : undefined} aria-label={`Open conversation with ${name}${isOnline ? ", online" : ""}`} className={`flex w-full min-w-0 items-center gap-3 rounded-2xl p-3 text-left transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-accent-hover ${isSelected ? "bg-accent shadow-soft" : "hover:bg-accent"}`}><PresenceAvatar profile={profile} isOnline={isOnline} /><div className="min-w-0 flex-1"><p className="truncate font-semibold text-heading">{name}</p><p className="mt-0.5 truncate text-sm text-body">{profile.username ? `@${profile.username}` : "Nemissive member"}</p></div><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-5 w-5 shrink-0 text-muted" aria-hidden="true"><path d="m9 6 6 6-6 6" strokeLinecap="round" strokeLinejoin="round" /></svg></button>;
          })}</div>
        )}
      </div>
    </div>
