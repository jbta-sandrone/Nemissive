...
function ProfileBannerPicker({ profile, accountStatus, premiumAccess, savedBorder, savedBanner, selection, isSaving, error, onSelectionChange, onApply }: ProfileBannerPickerProps) {
  ...
  function renderThemeOption(theme: ProfileThemeDefinition) {
    ...
    return (
      <button key={theme.key} type="button" aria-pressed={selected} aria-label={`${theme.name}, ${theme.access === "premium" ? "Elite profile theme" : "free profile theme"}, ${stateLabel}${selected ? ", selected" : ""}${equipped ? ", currently equipped" : ""}`} disabled={isSaving} onClick={() => onSelectionChange(theme.key)} className={`group relative min-w-0 rounded-2xl border p-2 text-left transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-accent-hover disabled:opacity-50 ${selected ? "border-primary bg-accent shadow-sm" : "border-border bg-surface hover:bg-accent/70"}`}>
    ...
    <div>
      ...
      {selectedDefinition.access === "premium" && selectedAccessSource && (
        <div className="mt-4 rounded-2xl border border-border bg-surface px-4 py-3">
          ...
          {selectedAccessSource === "locked" && <p className="mt-2 text-xs leading-5 text-body">Included with active Nemissive Elite. Individual purchasing is not available yet.</p>}
        </div>
      )}
