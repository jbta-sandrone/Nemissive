...
function MenuContent({ isArchived, isSaving, error, onProfile, onArchive, onDelete, onNicknames, onTheme, onContent }: { isArchived: boolean; isSaving: boolean; error: string; onProfile: () => void; onArchive: () => void; onDelete: () => void; onNicknames: () => void; onTheme: () => void; onContent: () => void }) {
  return <><div role="menu" className="space-y-1"><Item autofocus label="View profile" icon={icons.profile} onClick={onProfile} disabled={isSaving} /><Item label="Edit nicknames" icon={icons.nickname} onClick={onNicknames} disabled={isSaving} /><Item label="Change theme" icon={icons.theme} onClick={onTheme} disabled={isSaving} /><Item label="Media, files & links" icon={icons.content} onClick={onContent} disabled={isSaving} /><div role="separator" className="my-2 border-t border-border" /><Item label={isArchived ? "Unarchive conversation" : "Archive conversation"} icon={icons.archive} onClick={onArchive} disabled={isSaving} /><Item label="Delete chat" icon={icons.delete} onClick={onDelete} disabled={isSaving} destructive /><div role="separator" className="my-2 border-t border-border" /><FutureItem label="Block" icon={icons.block} /><FutureItem label="Report" icon={icons.report} /></div>{isSaving && <p role="status" className="px-3 pt-2 text-xs text-muted">Saving…</p>}{error && <p role="alert" className="mt-2 rounded-xl bg-accent px-3 py-2 text-xs text-body">{error}</p>}</>;
}
...
function ConversationOptionsMenu({ conversationId, conversationName, profile, profilePresenceText, isProfileOnline, isArchived, nicknamesByUserId, participants, currentTheme, onArchivedChange, onDelete, onNicknameSave, onThemeApply, onContentJump }: Props) {
  const triggerRef = useRef<HTMLButtonElement>(null); const [open, setOpen] = useState(false); const [desktop, setDesktop] = useState(() => matchMedia("(min-width: 768px)").matches); const [saving, setSaving] = useState(false); const [error, setError] = useState("");
