...
type Props = {
  ...
  profileAccountStatus: AccountStatus | null;
...
const icons = {
  ...
  voice: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className={iconClass} aria-hidden="true"><path d="M7.2 3.8 10 8l-2.1 2.1a14.2 14.2 0 0 0 6 6L16 14l4.2 2.8-.8 3.4c-.2.8-1 1.4-1.8 1.3C9.7 20.5 3.5 14.3 2.5 6.4c-.1-.8.5-1.6 1.3-1.8l3.4-.8Z" strokeLinecap="round" strokeLinejoin="round" /></svg>,
  video: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className={iconClass} aria-hidden="true"><rect x="3" y="6" width="13" height="12" rx="2.5" /><path d="m16 10 5-3v10l-5-3" strokeLinecap="round" strokeLinejoin="round" /></svg>,
};
...
function FutureItem({ label, icon, className = "flex" }: { label: string; icon: ReactNode; className?: string }) { return <button type="button" role="menuitem" disabled aria-disabled="true" aria-label={`${label}, coming soon`} className={`${className} min-h-11 w-full cursor-not-allowed items-center gap-3 rounded-xl px-3 py-2 text-left text-sm text-muted opacity-70`}><Icon>{icon}</Icon><span className="min-w-0 flex-1 font-semibold">{label}</span><span aria-hidden="true" className="rounded-lg bg-background px-2 py-1 text-[0.62rem] font-bold uppercase tracking-wide">Coming soon</span></button>; }
...
function ConversationOptionsMenu({ conversationId, conversationName, profile, profileAccountStatus, profilePresenceText, isProfileOnline, isArchived, isBlocked, isDeleted, messagingAvailable, isConnected, nicknamesByUserId, participants, currentTheme, premiumAccess, onArchivedChange, onDelete, onDisconnect, onNicknameSave, onThemeApply, onContentJump, onUserBlockChange }: Props) {
