...
function ComposerMediaPreview({ images, disabled, onRemove, onRemoveAll }: ComposerMediaPreviewProps) {
  ...
  return (
    <section aria-label={`${images.length} selected ${images.length === 1 ? "image" : "images"}`} className="mb-2 max-h-[min(42dvh,24rem)] overflow-y-auto overscroll-contain rounded-2xl border border-border bg-surface p-3 shadow-soft">
