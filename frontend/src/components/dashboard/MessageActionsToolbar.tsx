...
function MessageActionsToolbar({ canDelete, canEdit, canPin, canInteract, disabled, isOutgoing, isPinned, isPinPending, replyLabel, onDelete, onEdit, onPin, onReact, onReply }: MessageActionsToolbarProps) {
  return (
    <div className={`chat-message-actions absolute top-1/2 z-20 hidden -translate-y-1/2 items-center gap-0.5 rounded-2xl border border-border bg-surface p-1 shadow-soft transition-opacity duration-150 motion-reduce:transition-none md:flex md:opacity-0 md:group-hover/message:opacity-100 md:group-focus-within/message:opacity-100 ${isOutgoing ? "right-full mr-2" : "left-full ml-2"}`}>
      {canInteract && <button type="button" onClick={(event) => { event.stopPropagation(); onReply(); }} aria-label={replyLabel} title="Reply" className="chat-accent-control flex h-9 w-9 items-center justify-center rounded-xl text-muted transition hover:bg-accent hover:text-heading focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/20"><ReplyIcon /></button>}
      {canInteract && <button type="button" onClick={(event) => { event.stopPropagation(); onReact(event.currentTarget); }} aria-label="React to this message" title="React" className="chat-accent-control flex h-9 w-9 items-center justify-center rounded-xl text-muted transition hover:bg-accent hover:text-heading focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/20"><ReactionIcon /></button>}
      {canPin && <button type="button" onClick={(event) => { event.stopPropagation(); onPin(); }} disabled={isPinPending} aria-label={isPinned ? "Unpin this message" : "Pin this message"} title={isPinned ? "Unpin" : "Pin"} aria-pressed={isPinned} className={`chat-accent-control flex h-9 w-9 items-center justify-center rounded-xl transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/20 disabled:cursor-wait disabled:opacity-50 ${isPinned ? "chat-accent-control-active bg-accent text-primary" : "text-muted hover:bg-accent hover:text-heading"}`}><PinIcon filled={isPinned} /></button>}
      {canEdit && <button type="button" onClick={(event) => { event.stopPropagation(); onEdit(event.currentTarget); }} disabled={disabled} aria-label="Edit your message" title="Edit" className="chat-accent-control flex h-9 w-9 items-center justify-center rounded-xl text-muted transition hover:bg-accent hover:text-heading focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/20 disabled:cursor-wait disabled:opacity-50"><EditIcon /></button>}
